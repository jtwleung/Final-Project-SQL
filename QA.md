...
2. The significant amount of missing data/rows in some columns may be a problem for validity of the insights generated by data analysis.

3. It is possible that the data analysis is based on JOINs across tables using keys that are not the same "context" even if they have the same name or look similar.  My "Part B" shows some (not all) of the SQL I ran during EDA to examine the data and feel more confident doing JOINs using certain fields and across certain tables during analysis.

4. It is possible that data cleaning will inadvertently corrupt the "<tablename>_clean" version of the data, and QA checks are required to ensure integrity after cleaning.  This is detailed in my "Part A".
