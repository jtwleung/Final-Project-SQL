...
-- Do "on the fly" data cleaning to obtain a value for productname for missing values.  Default to products_clean.name and if that's missing, only then use allsessions_clean.v2productname
...
-- Further refine data set by ranking over city, country by number ordered
...
-- Final query to obtain result set showing most ordered product from each city, country combination
SELECT DISTINCT * 
...
-- Do "on the fly" cleaning of the sentiment score and sentimentmagnitude, using interpolation (filling in with avg, when missing a value)
...
-- Add in the calculation for the impact column, based on assumptions on an appropriate formula for impact
...
-- Further refine result set by adding SUM and AVG columns partitioned by country
...
-- Final query to show both rank by revenue and rank by impact score
SELECT
	country,
	total_revenue,
	country_impact,
